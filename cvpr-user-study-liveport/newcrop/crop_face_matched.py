...
Processes videos in FINAL_BASELINES_LIVEPORT/<method>/<method>_scale1.5/ directories.
...
def get_reference_for_subject(base_dir, sentence, subject):
    ...
    for method, subdir in reference_methods:
        ...
        video_pattern = f"{method}_liveport_{sentence}_{subject}.mp4"
...
def process_videos(methods_to_process=None):
    ...
    """
    base_dirs = [
        Path(__file__).parent / 'FINAL_BASELINES_LIVEPORT',
        Path(__file__).parent / 'FINAL_BRIGHT_LIVEPORT'
    ]
    reference_base_dir = Path(__file__).parent / 'FINAL_BASELINES_LIVEPORT'  # Always use this for reference
        ...
        for base_dir in base_dirs:
            ...
            if not input_method_dir.exists():
                ...
                for video_file in sorted(video_files):
                    ...
                    # Extract subject info from filename (e.g., ga_liveport_A_104_scale1.5.mp4 -> A, 104)
                    ...
                    if cache_key in crop_cache:
                        ...
                        # Get reference values for this specific subject (always from FINAL_BASELINES_LIVEPORT)
            ...
            if not input_rotated_dir.exists():
                ...
                for video_file in sorted(rotated_video_files):
                    ...
                    # Extract subject info from filename (e.g., ga_liveport_A_104_rotated_scale1.5.mp4 -> A, 104)
                    filename = video_file.stem  # Remove .mp4
                    parts = filename.split('_')
                    if len(parts) >= 4:
                        sentence = parts[2]  # A or B
