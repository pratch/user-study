...
for subject in "${subjects[@]}"; do
    for sen in "A" "B"; do
        ...
        # ------------------ Swapped: GA left, Ours right ------------------

        out_name_swapped="pair_${subject}_${sen}_ga_vs_ours"

        video_left_swapped=$ga_path/$ga_name
        video_right_swapped=$ours_path/$ours_name

        out_video_left_swapped=$ga_path/labeledA_$ga_name
        out_video_right_swapped=$ours_path/labeledB_$ours_name

        out_swapped=$out_path"/"$out_name_swapped

        # # Left video (Method A, now GA)
