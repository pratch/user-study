...
for subject in "${subjects[@]}"; do
    for sen in "A" "B"; do
        ...
        talkg_path=$base_path"/data_talkinggaussian_redrive_live"

        ours_name="${sen}_synctalk_222200037_nersemble_${subject}_2_200000"
        talkg_name="may${subject}_audio${sen}"

        out_name="pair_${subject}_${sen}_ours_vs_talkg"

        video_left=$ours_path/$ours_name
        video_right=$talkg_path/$talkg_name

        out_video_left=$ours_path/labeledA_$ours_name
        out_video_right=$talkg_path/labeledB_$talkg_name
        ...
        # ------------------ Swapped: TALKG left, Ours right ------------------

        out_name_swapped="pair_${subject}_${sen}_talkg_vs_ours"

        video_left_swapped=$talkg_path/$talkg_name
        video_right_swapped=$ours_path/$ours_name

        out_video_left_swapped=$talkg_path/labeledA_$talkg_name
        out_video_right_swapped=$ours_path/labeledB_$ours_name

        out_swapped=$out_path"/"$out_name_swapped

        # # Left video (Method A, now TALKG)
